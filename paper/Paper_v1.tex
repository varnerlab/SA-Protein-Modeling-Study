...
\caption{Head-to-head comparison of SA generation against learned baselines across all seven Pfam families. \textbf{(A)}~KL divergence of amino acid composition. \textbf{(B)}~Novelty in PCA space. \textbf{(C)}~Nearest sequence identity. Significance brackets show Wilcoxon rank-sum tests comparing SA generation to HMM emit (the most direct comparison, as both use only the seed alignment): $^{***}p < 0.001$, $^{**}p < 0.01$, $^{*}p < 0.05$, n.s.\ = not significant. Error bars show $\pm 1$ SE (30 chains $\times$ 5 samples).}
