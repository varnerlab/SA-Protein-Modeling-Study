...
\significancestatement{Most protein families have fewer than 100 known members, too few to train a deep generative model. We show that a single attention operation over a family alignment defines an energy landscape whose Boltzmann distribution can be sampled exactly via Langevin dynamics, with no training, no GPU, and no external data. The resulting method, stochastic attention, generates novel protein sequences that preserve family-level amino acid statistics and are predicted by independent structure predictors to adopt the correct fold across eight diverse protein families, including candidate antimicrobial peptides. A simple linear relationship between the dimensionality of the sequence representation and the critical sampling temperature enables fully automatic operation from a seed alignment, opening training-free sequence generation to the majority of protein families that are too small for conventional deep learning.}
...
\correspondingauthor{\textsuperscript{1}To whom correspondence should be addressed. E-mail: jdv27@cornell.edu}
...
\begin{equation}\label{eq:score}
    \nabla \log p_\beta(\boldsymbol{\xi}) = \beta\bigl(\mathbf{T}(\boldsymbol{\xi}) - \boldsymbol{\xi}\bigr),
\end{equation}
where $\mathbf{T}(\boldsymbol{\xi}) := \mathbf{X}\,\operatorname{softmax}\!\bigl(\beta\,\mathbf{X}^\top\boldsymbol{\xi}\bigr)$ is the softmax attention retrieval map. This score is \emph{exact} and computed by a single attention operation; no score network or training is required.

\subsection*{Stochastic Attention Update.}
Applying the unadjusted Langevin algorithm (ULA)~\cite{robertsTweedieExponentialConvergence1996} yields the stochastic attention update:
...
\end{equation}
Each step performs: (i) contraction toward the origin, (ii) a softmax-weighted pull toward stored memories, and (iii) isotropic Gaussian noise. The per-step cost is $\mathcal{O}(dK)$.

\subsection*{Protein Families and Experimental Setup.}
We selected eight Pfam families (Table~\ref{tab:families}): RRM, SH3, WW, Kunitz, zf-C2H2, PDZ, Pkinase, and Defensin\_beta. Seed alignments were downloaded from InterPro~\cite{mistryPfamProteinFamilies2021} and cleaned by removing columns with $>50\%$ gaps and sequences with $>30\%$ gaps. PCA retained 95\% of variance, followed by unit-norm projection.

For each family, we ran 30 independent ULA chains of $T{=}5{,}000$ iterations ($\alpha{=}0.01$), discarding $2{,}000$ burn-in iterations and thinning every 100 steps to retain 5 samples per chain ($S{=}150$ sequences). The inverse temperature was set automatically via the entropy inflection criterion. We defined a generation regime at $\beta_{\mathrm{gen}} \approx 2\beta^*$ and a retrieval regime at $\beta_{\mathrm{ret}} \approx 20\beta^*$.

\subsection*{Baselines.}
We compared: (i) profile HMM emit via HMMER3~\cite{hmmer3}; (ii) EvoDiff MSA-conditioned diffusion~\cite{alamdariProteinGeneration2023}; (iii) MSA Transformer Gibbs sampling~\cite{raoMSATransformer2021}; and (iv) Potts model via plmDCA. For each baseline, the same 150-sequence budget and evaluation metrics were applied.

\subsection*{Structure Validation.}
We predicted structures for 50 sequences per method per family using ESMFold~\cite{linEvolutionaryScaleModeling2023} and AlphaFold2~\cite{jumperHighlyAccurateProtein2021} (via ColabFold~\cite{mirdita2022colabfold}). Structures were compared to experimentally determined references using TM-align~\cite{zhangTMalign2005}. Statistical comparisons used Wilcoxon rank-sum tests with Benjamini-Hochberg false discovery rate (FDR) correction at $q{=}0.05$.
}

\acknow{The idea for stochastic attention arose while developing a lecture on energy-based methods for eCornell. We thank the Pfam/InterPro consortium for maintaining publicly available seed alignments. Structure predictions were performed using ESMFold and AlphaFold2 (via ColabFold). Computations were performed on resources at Cornell University. All code, data, and analysis scripts are available at \url{https://github.com/varnerlab/SA-Protein-Modeling-Study}.}
...
\caption{Cross-family generation quality across eight Pfam families. \textbf{(A)}~KL divergence of amino acid composition. \textbf{(B)}~Novelty in PCA space. \textbf{(C)}~Nearest sequence identity to a stored pattern. Error bars: $\pm 1$ SE (30 chains $\times$ 5 samples).}
...
\caption{Predicting the critical temperature from MSA statistics. \textbf{(A)}~Predicted vs.\ empirical $\beta^*$ ($R^2{=}0.97$, $n{=}33$). Blue circles: Pfam families; gold diamonds: WW scaling replicates. \textbf{(B)}~$\beta^*$ vs.\ $\sqrt{d}$, with fitted regression (dashed) and random-pattern prediction (dotted red). \textbf{(C)}~Model comparison (log-scale root-mean-square error [RMSE]).}
...
\caption{Head-to-head comparison of SA against three learned baselines across all eight families. \textbf{(A)}~KL divergence. \textbf{(B)}~Novelty. \textbf{(C)}~Nearest sequence identity. Per-chain means $\pm 1$ SE.}
...
\caption{Structure validation using ESMFold. \textbf{(A)}~Mean pLDDT across eight families. Dashed line: pLDDT${=}70$ confidence threshold. \textbf{(B)}~TM-score to experimentally determined reference structures. Dashed line: TM${=}0.5$ same-fold threshold. Significance: Wilcoxon rank-sum, $^{***}p < 0.001$, $^{**}p < 0.01$, $^{*}p < 0.05$, n.s.\ = not significant. $n{=}50$ sequences per condition.}
\label{fig:structure-validation}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{figs/defensin_biophysics.pdf}
\caption{Biophysical properties of beta defensin sequences. \textbf{(A)}~Net charge vs.\ hydrophobic fraction. \textbf{(B)}~Cysteine count distribution. \textbf{(C)}~Net charge distribution. \textbf{(D)}~Antimicrobial plausibility filter pass rate.}
...
\caption{AlphaFold2 predicted structures for representative SA-generated sequences (colored by pLDDT) superimposed on experimentally determined reference structures (gray). Sub-angstrom predicted backbone RMSD despite $40$--$65\%$ sequence novelty.}
